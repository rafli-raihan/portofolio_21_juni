...
$queryMe = mysqli_query($connection, "SELECT * FROM admin LIMIT 1"); // LIMIT buat ngebatasin berapa banyak data yg di query / fetch dari db, misal LIMIT 3 ya dia fetch 3 data aj dari keseluruhan tabel gak kya sebelumnya yg tanpa limit    
...
if (isset($_POST['simpan'])) {
    ...
    if ($row) {
        # update
        $myId = $row['id']; // $row['nama_kolom_yg_mw_diambil'] ini buat ambil value dari satu kolom dari row, nahh biasanya ini buat ngambil id
        $update = mysqli_query($connection, "UPDATE settings SET name='$name', email='$email', 
        password='$password', profile_pic='$profile_pic_name', fb='$fb', 
        ig='$ig', github='$github', linkedin='$linkedin' WHERE 
        id='$myId'");
